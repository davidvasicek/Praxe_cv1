...
String pracoviste = "ucitel1"; // číslo pracoviště; nahraďte x číslem vašeho pracoviště
 
const char* ssid = "Uc17_IoT";         // Název WiFi sítě; SSID
const char* password = "teleinformatika";   // Heslo WiFi sítě; password
...
void setup(){
 ...
 clientMQTT.begin("192.168.0.100", clientWiFi);
...
void connectToMQTT() {
  ...
  while (!clientMQTT.connect("ucitel1")) {
    Serial.print(".");
  }
 
 Serial.println("klient MQTT připojen"); // výpis na obrazovku
 
 clientMQTT.subscribe(pracoviste + "_control"); // přihlášení se k odběru konkrétního tématu; budu odebírat BSS/pracoviste_x
}
...
void loop() {
  ...
  if(millis() - posledniCasOdeslani > 5000) {
    ...
    StaticJsonBuffer<64> JsonBuffer; // vytvoření statického JSON bufferu o velikosti 128 bytů; možno vytvořit i buffer dynamicky 
